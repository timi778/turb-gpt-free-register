...
class RemailProviderTests(unittest.TestCase):
    def test_parse_sources_keeps_remail_in_order(self):
        self.assertEqual(
            email_provider.parse_email_sources("outlook,remail,generic_api"),
            ["outlook", "remail", "generic_api"],
        )
    ...
    @patch("core.email_provider.resolve_email_source", return_value="remail")
    def test_release_email_clears_remail_context(self, resolve, release_account):
