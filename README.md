...
- 管理账号、邮箱池、Codex 凭证；账号页支持按注册批次分组筛选、编辑/删除分组，以及仅对当前分组复制全部 Token/整行。
- 配置页支持热加载，保存后无需重启。
...
WebUI 的「异常补登」页会通过管理接口读取 chatgpt2api 已判定为异常的邮箱，并按邮箱匹配本地注册记录。只有本地存在且保存了 Remail 长效收件凭据的账号可以补登；本地无记录、非长效邮箱和正在补登的账号会标记原因并自动跳过。「补登全部可用」会连续处理当前异常列表，某个账号失败不会中断批次，后续账号仍会继续补登。
